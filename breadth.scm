...
(define (spy msg x) (display msg) (display ": ") (display x) (newline) x) ; depuración
...
(define (buscar bc fin grafobusq estexp)
	(spy "entra grafo" grafobusq) ; depuración
	(cond ((null? grafobusq) (fracaso))
		  ((pertenece fin (car grafobusq)) (exito grafobusq))
		  (#t (buscar bc fin (spy "sigue grafo" (append (cdr grafobusq) (expandir (car grafobusq) bc estexp))) 
